...
docker run --user root --rm \
  --cap-add=SYS_ADMIN --cap-add=PERFMON --cap-add=SYS_PTRACE \
  --security-opt apparmor=unconfined --security-opt seccomp=unconfined \
  --pid=host --privileged \
  -p 33000:3000 \
  -e ADMIN_GROUPS="admin,globalaccess" \
  -e SWAGGER_PATH=/backend/explorer \
  -e MONGODB_URI=mongodb://db:27017/dacat \
  ...
  --link mongo:db --link apm:apm \
  --network root_elastic \
  --name backend00 \
  back 0x --output-dir=/home/node/app/profiles -q --collect-delay=10 -- node dist/main
#frontend
